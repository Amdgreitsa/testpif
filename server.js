...
function canAccessAccount(user, account) { return Boolean(account) && (user.role === 'admin' || account.userId === user.id); }
function accountForUser(db, user, accountId) { const account = accountId ? db.accounts.find(item => item.id === accountId) : db.accounts.find(item => canAccessAccount(user, item) && !item.isTelegramPlaceholder); return canAccessAccount(user, account) && !account.isTelegramPlaceholder ? account : null; }
...
function analytics(db, user) {
  const accounts = db.accounts.filter(a => (user.role === 'admin' || a.userId === user.id) && !a.isTelegramPlaceholder);
...
function apifyEndpoint(path) { const url = new URL(`https://api.apify.com/v2/${path}`); url.searchParams.set('token', APIFY_TOKEN); return url; }
const delay = milliseconds => new Promise(resolve => setTimeout(resolve, milliseconds));
async function apifyJson(path, options) { const response = await fetch(apifyEndpoint(path), options); const text = await response.text(); let data; try { data = text ? JSON.parse(text) : null; } catch { throw new Error(`Apify вернул неожиданный ответ (${response.status})`); } if (!response.ok) throw new Error(data?.error?.message || data?.error || `Apify вернул ${response.status}`); return data; }
async function runApify(input) {
  if (!APIFY_TOKEN) throw new Error('APIFY_TOKEN не настроен на сервере');
  let lastError;
  for (let attempt = 0; attempt < 3; attempt += 1) {
    try {
      const started = await apifyJson(`acts/${encodeURIComponent(APIFY_ACTOR)}/runs`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify(input) });
      const runId = started?.data?.id;
      if (!runId) throw new Error('Apify не вернул идентификатор запуска');
      const deadline = Date.now() + 9 * 60 * 1000;
      let run = started.data;
      while (!['SUCCEEDED', 'FAILED', 'ABORTED', 'TIMED-OUT'].includes(run.status) && Date.now() < deadline) { await delay(1000); run = (await apifyJson(`actor-runs/${encodeURIComponent(runId)}`, { headers: { accept: 'application/json' } }))?.data; if (!run) throw new Error('Apify не вернул состояние запуска'); }
      if (run.status !== 'SUCCEEDED') throw new Error(run.status === 'RUNNING' ? 'Apify не завершил импорт за 9 минут' : `Apify завершил импорт со статусом ${run.status}`);
      if (!run.defaultDatasetId) throw new Error('Apify не вернул набор данных');
      const items = []; const pageSize = 1000;
      for (let offset = 0; ; offset += pageSize) { const page = await apifyJson(`datasets/${encodeURIComponent(run.defaultDatasetId)}/items?clean=true&offset=${offset}&limit=${pageSize}`, { headers: { accept: 'application/json' } }); if (!Array.isArray(page)) throw new Error('Apify вернул неожиданный формат данных'); items.push(...page); if (page.length < pageSize) break; }
      return items;
...
http.createServer(async (req, res) => {
  try {
    ...
    if (url.pathname === '/api/health') return send(res, 200, { ok: true, apifyConfigured: Boolean(APIFY_TOKEN), telegramConfigured: telegramEnabled() });
    ...
    if (url.pathname === '/api/dashboard' && req.method === 'GET') { const signed = auth(req, res, db); if (!signed) return; const data = analytics(db, signed); const ids = new Set(db.accounts.filter(a => (signed.role === 'admin' || a.userId === signed.id) && !a.isTelegramPlaceholder).map(a => a.id)); return send(res, 200, { analytics: data, reels: db.reels.filter(r => ids.has(r.accountId)).sort((a, b) => new Date(b.publishedAt) - new Date(a.publishedAt)).map(reelDto), accounts: db.accounts.filter(a => ids.has(a.id)) }); }
