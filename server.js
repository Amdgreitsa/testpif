...
function accountForUser(db, user, accountId) { const account = accountId ? db.accounts.find(item => item.id === accountId) : db.accounts.find(item => canAccessAccount(user, item)); return canAccessAccount(user, account) ? account : null; }
