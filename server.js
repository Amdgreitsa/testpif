...
async function fetchInstagram(url) {
  ...
  const endpoint = `https://api.apify.com/v2/acts/${encodeURIComponent(APIFY_ACTOR)}/run-sync-get-dataset-items?token=${encodeURIComponent(APIFY_TOKEN)}`;
  const response = await fetch(endpoint, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ directUrls: [url], resultsType: 'posts', resultsLimit: 1 }) });
  if (!response.ok) throw new Error(`Apify вернул ${response.status}`);
  const [item] = await response.json(); if (!item) throw new Error('Apify не вернул данные по этой ссылке'); return item;
}
function normalize(item, url) { return { sourceUrl: url, title: (item.caption || 'Новый рилс').replace(/\s+/g, ' ').slice(0, 90), publishedAt: item.timestamp || new Date().toISOString(), views: Number(item.videoViewCount || item.videoPlayCount || 0), likes: Number(item.likesCount || 0), comments: Number(item.commentsCount || 0), duration: Number(item.videoDuration || 0), coverUrl: item.displayUrl || item.thumbnailUrl || null }; }
