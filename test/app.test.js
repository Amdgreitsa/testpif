...
test('installer normalizes a copied URL and the browser never embeds an Apify secret', () => {
  ...
  assert.match(server, /api\.apify\.com/); assert.match(server, /APIFY_TOKEN/); assert.match(server, /syncConfiguredAdmin/); assert.match(server, /verifyTelegramAuth/); assert.match(server, /TELEGRAM_BOT_TOKEN/); assert.match(server, /auth\/telegram\/login/); assert.match(server, /telegramId/); assert.match(server, /role: 'creator'/); assert.doesNotMatch(server, /require\(['"](?:qrcode|telegram)/); assert.doesNotMatch(server, /TELEGRAM_API_HASH|TELEGRAM_ALLOWED_USER_IDS/); assert.deepEqual(packageManifest.dependencies || {}, {}); assert.match(install, /certbot/); assert.match(install, /systemctl enable/); assert.match(install, /APP_DIR="\/opt\/pifpaf-creators"/); assert.match(install, /tar --exclude='\.\/data'/); assert.match(install, /--exclude='\.\/node_modules'/); assert.match(install, /nodejs npm/); assert.match(install, /command -v npm/); assert.match(install, /command -v fuser/); assert.match(install, /wait_for_package_manager/); assert.match(install, /DPkg::Lock::Timeout/); assert.match(install, /npm ci --omit=dev --prefix/); assert.match(install, /TELEGRAM_BOT_TOKEN/); assert.doesNotMatch(install, /TELEGRAM_API_HASH|TELEGRAM_ALLOWED_USER_IDS/); assert.match(install, /listen 443 ssl/); assert.match(install, /ssl_certificate/); assert.match(install, /wait_for_public_https/); assert.match(install, /https:\/\/\$domain\/api\/health/); assert.match(install, /curl nginx/); assert.match(install, /while true; do/); assert.match(install, /normalize_domain/); assert.match(install, /--update/); assert.match(install, /check_dns/); assert.match(install, /wait_for_health/); assert.match(app, /onTelegramAuth/); assert.match(app, /api\/accounts\/import/); assert.match(html, /telegramLogin/); assert.doesNotMatch(app, /apify_api_/); assert.doesNotMatch(html, /APIFY_TOKEN|APIFY_ACTOR|SYNC_INTERVAL_MS|\.env|переменные окружения/);
});
