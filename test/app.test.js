...
test('installer normalizes a copied URL and the browser never embeds an Apify secret', () => {
  ...
  assert.match(server, /api\.apify\.com/); assert.match(server, /APIFY_TOKEN/); assert.match(install, /certbot/); assert.match(install, /systemctl enable/); assert.match(install, /APP_DIR="\/opt\/pifpaf-creators"/); assert.match(install, /cp -a "\$SOURCE_DIR"/); assert.match(install, /curl nginx/); assert.match(install, /while true; do/); assert.match(install, /DOMAIN="\$\{DOMAIN#https:\/\/\}"/); assert.match(install, /DOMAIN="\$\{DOMAIN%%\/\*\}"/); assert.doesNotMatch(app, /apify_api_/);
});
